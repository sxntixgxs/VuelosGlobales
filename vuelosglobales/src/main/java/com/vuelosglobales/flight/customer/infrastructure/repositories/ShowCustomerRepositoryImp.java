...
public class ShowCustomerRepositoryImp implements ShowCustomerRepository{
    ...
    public List<String> getCustomerList(String id) {
        String query = "SELECT C.id,CONCAT(C.name,' ',C.surname) AS name, C.age, TP.typeDoc FROM customer C INNER JOIN typeDocument TP ON C.idTypeDoc = TP.id WHERE C.id = ?";
        ...
        try(
            ...
            if(resultSet.next()){
                ...
                customerList.add("Customer name: "+resultSet.getString("name"));
                customerList.add("Customer age: "+Integer.toString(resultSet.getInt("C.age")));
            }
            return customerList;
